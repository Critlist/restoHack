...
# - Runs ./dev/prepare-release.sh with the new version
...
#   - ./dev/prepare-release.sh present & executable in repo root
#
# Notes:
...
ensure_prepare_exists() {
  [[ -x ./dev/prepare-release.sh ]] || die "./dev/prepare-release.sh not found or not executable."
}
...
run_prepare() {
  local tag="$1"
  if [[ "${DRY_RUN}" == "true" ]]; then
    echo "[dry-run] ./dev/prepare-release.sh ${tag}"
    return 0
  fi
  ./dev/prepare-release.sh "${tag}"
}
