...
/**
 * MODERN ADDITION (2026): Windows platform support implementation
 *
 * This file contains all Windows-specific code for 1984 Hack, isolated
 * from the original codebase to preserve historical authenticity.
 *
 * Implements:
 * - PDCurses termcap compatibility shim (uses PDCurses API, not ANSI escapes)
 * - File locking via Windows LockFileEx (replaces Unix flock)
 * - Console control handler for Ctrl+C and window close events
 * - UTF-8 console setup for proper box-drawing character rendering
 */
...
/* Modern: Initialize PDCurses and set UTF-8 console mode */
int tgetent(char *bp, const char *name) {
  ...
  if (!pdcurses_initialized) {
    ...
    /* Modern: Set UTF-8 console mode for box-drawing characters
     * CRITICAL: Both input and output codepages required to prevent
     * rendering bugs and input edge cases on various Windows configurations */
...
/* Modern: Return symbolic capability strings
 * These are interpreted by tputs(), not output directly */
...
/* Modern: Return terminal dimensions from PDCurses */
...
/* Modern: Format cursor motion string with coordinates */
...
/* Modern: Execute terminal operations via PDCurses API
 * CRITICAL: This calls PDCurses functions, not putchar() */
...
/* Modern: Lock game for single-player access */
...
/* Modern: Release game lock and cleanup handle */
...
/* Modern: Windows console control handler */
